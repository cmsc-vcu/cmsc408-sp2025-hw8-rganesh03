...
run_sql_and_return_html(cnx,"""
select `short name`, `long name`, region, `income group`
from sp25_ganeshr_user.wdi_country
...
run_sql_and_return_html(cnx,"""
select
count(*) as "Number of Countries", region
from sp25_ganeshr_user.wdi_country
...
select region, `income group`,
       ...
       ((select count(`income group`) 
        ...
        (select count(*) 
         from wdi_country) * 100) as "Percentage of Region"
from wdi_country
group by region, `income group`;
""")
...
select 
    ...
    (count(*) * 100.0 / 
     (select count(*) 
      from wdi_country as reg 
      where reg.region = wdi_country.region)) as "Percent of Region-Income Group" 
from wdi_country
group by region, `income group`;
""")
```
...
select 
    ...
    ((select count(*) 
      ...
      where inc.`income group` = wdi_country.`income group`) 
      / (select count(*) 
         from wdi_country) * 100) as "Percent of Total"
from wdi_country
group by `income group`;
""")
